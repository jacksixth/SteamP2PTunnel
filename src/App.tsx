...
function App() {
    ...
    // 【修改 1/3】直接从 networkStatus 中解构出真实的 ping
    const { isConnected, statusMessage, ping } = networkStatus;

    // 【修改 2/3】删除下面这一行模拟 ping 的代码
    // const ping = isConnected ? Math.floor(Math.random() * 10) + 5 : 0;
    ...
    return (
        <div className="h-screen w-screen flex flex-col font-sans select-none overflow-hidden">
            ...
            <header className="h-16 glass-panel border-b-0 border-b-white/5 z-50 flex items-center justify-between px-6 shrink-0">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <div className="text-xs text-slate-400 flex items-center gap-2">
                            <span className={`w-2 h-2 rounded-full ${isConnected ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]' : 'bg-slate-600'}`}></span>
                            {statusMessage}
                        </div>
                    </div>
                </div>
                ...
                {isConnected && (
                    <div className="flex items-center gap-6 text-sm">
                        <div className="flex items-center gap-2 text-slate-300 bg-slate-800/50 px-3 py-1.5 rounded-full border border-white/5">
                    ...
                    <div className="lg:col-span-8 h-full min-h-0">
                        <div className="glass-panel rounded-2xl h-full flex flex-col overflow-hidden">
                            <div className="p-6 border-b border-white/5 flex items-center justify-between shrink-0">
                                ...
                                {isConnected && (
                                    <span className="text-xs bg-blue-500/20 text-blue-300 px-2 py-1 rounded border border-blue-500/20">
                                        P2P Encrypted
                                    </span>
                                )}
                            </div>
