...
import { Gamepad2, ArrowRight, Link } from "lucide-react";
    ...
    return (
        <div className="space-y-6">
            <div className="flex p-1 bg-slate-950/50 rounded-xl border border-white/5">
                <button onClick={() => setActiveTab('host')} className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${activeTab === 'host' ? 'bg-slate-800 text-white shadow-sm ring-1 ring-white/10' : 'text-slate-400 hover:text-slate-200'}`}>
                    我是房主
                </button>
                <button onClick={() => setActiveTab('join')} className={`flex-1 py-2 text-sm font-medium rounded-lg transition-all ${activeTab === 'join' ? 'bg-slate-800 text-white shadow-sm ring-1 ring-white/10' : 'text-slate-400 hover:text-slate-200'}`}>
                    加入游戏
                </button>
            </div>
            ...
            {activeTab === 'host' ? (
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    <div>
                        <label className="block text-xs font-medium text-slate-400 mb-1.5 uppercase tracking-wider">本地游戏端口</label>
                ...
                <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                    ...
                    <div>
                        <label className="block text-xs font-medium text-slate-400 mb-1.5 uppercase tracking-wider">通过房间 ID 加入</label>
                        <div className="flex items-center gap-2">
                            ...
                            <button onClick={() => handleJoinLobby(lobbyIdInput)} disabled={loading || !lobbyIdInput} className="btn-primary bg-green-600 hover:bg-green-500 group h-11 px-4">
                                <ArrowRight size={18}/>
                            </button>
                        </div>
                    </div>
